...
def train(model_path: str, total_timesteps: int, use_preset: bool = False):
    ...
    model = sac(policy="MlpPolicy", env=env, verbose=1, tensorboard_log=log_dir)
    ...
    for i in range(1,100):
        ...
        model.learn(total_timesteps=TIME_STEPS, reset_num_timesteps=False, tb_log_name="sac")
        model.save(f"sac{path_extent}/steps={TIME_STEPS*i}")
...
def visualize(model_path: str, use_preset: bool = False, random_terrain: bool = False):
    if random_terrain and use_preset:
        ...
        model = sac.load(f"sac_quadex/steps={int(mode_number)*10000}", env=env)
    elif use_preset:
        env = gym.make('Ant-v5', render_mode="human")
        mode_number = input("Enter model stepcount (sac_Ant-v5/steps=x*10000): ")
        model = sac.load(f"sac_Ant-v5/steps={int(mode_number)*10000}", env=env)
    else:
        env = QuadrupedEnv(model_path, render_mode="human")
        model_number = input("Enter model stepcount (sac_quad_ex/steps=x*10000): ")
        model = sac.load(f"sac_quad_ex/steps={int(model_number)*10000}", env=env)
    for ep in range(5):
        total_reward = 0
        obs, _ = env.reset()
